...
Public Class FFMOnayla
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If Not IsPostBack Then
            ...
            RadGrid1.DataSource = RSQL.GetValue(" SELECT BoschFMOnay.IdNr, isnull(SicilNo,0) AS SicilNo, isnull(Adi,'') + ' ' + isnull(Soyadi,'') AS AdSoyad, isnull(Per_Grp4,'') AS MasrafYeri, BoschFMOnay.Tarih, Baslangic, Bitis, Sure, '' AS Onay, '' AS Aciklama FROM BoschFMOnay WITH (NOLOCK) INNER JOIN BoschPerTab  WITH (NOLOCK)  ON Sicilno=Per_PersNr  JOIN  BoschLimit WITH (NOLOCK)  ON BoschLimit.Kodu=BoschPerTab.Per_Grp0 AND BoschPerTab.Per_Grp3=BoschLimit.Kodu2 WHERE  isnull(BoschFMOnay.iptal,0)<>1 AND BoschFMOnay.Tarih>=@BasTar AND BoschFMOnay.Tarih<=@BitTar AND Onay is null AND FMTipi=1 And BoschFMOnay.Tarih BETWEEN BoschLimit.OnayBasTar And BoschLimit.OnayBitTar AND SicilNo IN (" & Session("Secilen") & ") ORDER BY SicilNo, BoschFMOnay.Tarih ", {New SqlParameter("@BasTar", BasTar.SelectedDate.Value), New SqlParameter("@BitTar", BitTar.SelectedDate.Value)})
            RadGrid1.Rebind()
